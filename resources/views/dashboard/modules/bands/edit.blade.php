...
@section('right_pane')
	...
	<form action="{{ route('bands.update', $band->id) }}" class="w-100" enctype="multipart/form-data" method="POST">
		...
		<div class="form-group">
			...
			<input type="text" class="form-control" id="band_name" name="name" value="{{ $band->name }}" required>
		</div> {{-- /form-group --}}
		<div class="form-group">
			<div class="form-row">
				<div class="col">
					<label for="band_city">City</label>
					<input type="text" class="form-control" id="band_city" name="city" value="{{ $band->city }}" required>
				</div> {{-- /col --}}
				<div class="col">
		...
		<div class="form-group">
			<div class="form-row">
				<div class="col">
					...
					<input id="band_url" class="form-control" name="url" value="{{ $band->website_url }}">
				</div> {{-- /col --}}
				<div class="col">
					<label for="band_email">Email</label>
					<input id="band_email" class="form-control" name="email" value="{{ $band->email }}">
				</div> {{-- /col --}}
			</div>	{{-- /row --}}
		</div> {{-- /form-group --}}
